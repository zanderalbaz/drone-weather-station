...
{
	bool sim_mode = getenv("SIM") && 
					 (toupper(*getenv("SIM")) == '1' || 
					 toupper( *getenv("SIM"))== 'Y');
    ...
    if (!sim_mode) {
        ...
        returnCode = DjiCore_SetAlias("TriSonica Mini App");
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
		...
		USER_LOG_INFO("** SIMULATION ** - DJI core initialisation skipped");
    }
    ...
    T_DjiReturnCode ret = DjiTest_FcSubscriptionRunSample();
    if (ret != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("FC subscription sample failed with error code: %d", ret);
        return ret;
    }
