...
T_DjiReturnCode DjiTest_WidgetInteractionStartService(void)
{
    ...
    // Use our custom widget configuration directory that contains only our minimal JSON file
    char tempPath[WIDGET_DIR_PATH_LEN_MAX];
    snprintf(tempPath, WIDGET_DIR_PATH_LEN_MAX, "/home/rmbl/my_widget_config/");
                                                    ...
                                                    void *userData)
{
    ...
    // We expect our custom widget is the switch at index 1
    if (widgetType == DJI_WIDGET_TYPE_SWITCH && index == 1) {
        if (value == 1) {
            // Start logging – ensure startLogging() is accessible (from test_fc_subscription.c)
            startLogging();
        } else {
            // Stop logging
            stopLogging();
